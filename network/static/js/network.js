...
function showInfoCard(params) {
  ...
  for(var i in node["authors"]){
    if (i == node["authors"].length - 1) str += node["authors"][i]
    else str += node["authors"][i] + ", "
  }
  authors.html(str);
  
  var str="";
  for(var i in node["keywords"]){
    if (i == node["keywords"].length - 1) str += node["keywords"][i]
    else str += node["keywords"][i] + ", "
  }
  keywords.html(str);
