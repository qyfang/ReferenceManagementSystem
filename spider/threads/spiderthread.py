...
class ReferenceSpiderThread(threading.Thread):
    ...
    def saveModel(self, crawlresult, rids):
        ...
        abstract = crawlresult['abstract']

        obj,created = Reference.objects.get_or_create(selfid=selfid, title=title, 
            authors=authors, keywords=keywords, referids=referids, url=url, 
            abstract=abstract)
